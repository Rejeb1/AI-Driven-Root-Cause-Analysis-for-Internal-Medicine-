...
def emit(root: Path, limit: int | None) -> None:
    ...
    # Only replace numbers that already exist. DDXPlus knows that 64% of its
    # pneumonia patients smoke, but the fixture entry for pneumonia does not
    # carry a smoking likelihood at all, and adding one changes the shape of
    # the model rather than sourcing it -- every measurement taken so far was
    # on the current shape. Extending the vocabulary is a separate decision.
    fixture = build_knowledge_base()
    ...
    for label, pathologies in DISEASE_MAP.items():
        ...
        for concept, code in CONCEPT_MAP.items():
            ...
            if entry is None or concept not in entry.features:
                uncharacterised.append(f"{label}/{concept}")
                continue
    ...
    if uncharacterised:
        print(
            f"\n# {len(uncharacterised)} pairs skipped: the fixture entry does "
            f"not carry that\n# likelihood at all, so there is nothing to "
            f"replace. Adding it would extend\n# the model rather than source "
            f"it, which is a separate decision.",
            file=sys.stderr,
        )
